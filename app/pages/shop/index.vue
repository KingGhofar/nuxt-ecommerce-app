    ...
    <div class="flex flex-col sm:flex-row gap-4 mb-6">
      <div class="relative flex-1">
        ...
        <button 
          v-if="search" 
          @click="search = ''" 
          class="absolute right-4 top-1/2 -translate-y-1/2 text-earth-40 hover:text-earth transition-colors"
        >
          ✕
        </button>
      </div>
      <div class="relative sm:w-52">
        <select 
          v-model="sortBy" 
          class="input-field w-full appearance-none cursor-pointer pr-10"
        >
          <option v-for="opt in sortOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
        </select>
        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
          <ChevronDown class="w-5 h-5 text-earth-60" />
        </div>
      </div>
    </div>
